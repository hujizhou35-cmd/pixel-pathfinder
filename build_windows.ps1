...
$releaseBaseName = "PixelPathfinder_v2.0.0_Windows_x64"
$releaseExe = Join-Path $releaseDir ($releaseBaseName + ".exe")
$releaseZip = Join-Path $releaseDir ($releaseBaseName + ".zip")
...
try {
    ...
    if (-not $SkipSmokeTest) {
        Invoke-GodotStep "SMOKE" @("--headless", "--path", $projectRoot, "res://test/smoke_test.tscn")
    }
    if (-not $SkipVisualTest) {
        Invoke-GodotStep "VISUAL" @("--headless", "--path", $projectRoot, "res://test/visual_integration_test.tscn")
    }
...
$packageStage = Join-Path $buildDir $releaseBaseName
...
Copy-Item -LiteralPath $outputExe -Destination (Join-Path $packageStage ($releaseBaseName + ".exe"))
Copy-Item -LiteralPath (Join-Path $projectRoot "LICENSE") -Destination (Join-Path $packageStage "LICENSE")
Copy-Item -LiteralPath (Join-Path $projectRoot "THIRD_PARTY_NOTICES.md") -Destination (Join-Path $packageStage "THIRD_PARTY_NOTICES.md")
Copy-Item -LiteralPath (Join-Path $projectRoot "licenses") -Destination (Join-Path $packageStage "licenses") -Recurse
@"
Pixel Pathfinder v2.0.0 - Windows x64

Run $releaseBaseName.exe. No installer is required.
The executable uses an embedded PCK and must not create a sidecar .pck file.
Project source and verification details:
https://github.com/hujizhou35-cmd/pixel-pathfinder
"@ | Set-Content -LiteralPath (Join-Path $packageStage "README.txt") -Encoding UTF8

Compress-Archive -LiteralPath $packageStage -DestinationPath $releaseZip -CompressionLevel Optimal
Remove-Item -LiteralPath $packageStage -Recurse -Force
...
@(
    ...
    "$releaseZipHash  $([System.IO.Path]::GetFileName($releaseZip))"
...
$result = [ordered]@{
    ...
    release_zip_sha256 = $releaseZipHash
    release_hashes = $releaseHashes
}
$result | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $auditDir "Build_Windows_Result.json") -Encoding UTF8
